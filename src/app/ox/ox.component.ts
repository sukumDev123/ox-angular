...
export class OxComponent implements OnInit {
  private defaultVal = () => [['a' , 'b' , 'c'],['d' , 'e' ,'f'] ,['g' , 'h' , 'i']]
  private defaultCss = () => [['none' , 'none' , 'none'] ,['none' , 'none' , 'none'] , ['none' , 'none' , 'none']]
  showIs = false
  boxs = [1, 2 , 3]
  ...
  checkValFunc(ind , ind2) {
    return this.box_script[ind][ind2] == 'x' || this.box_script[ind][ind2] == 'o' ? true : false
  }
  onClickBox(ind , ind2) {
  ...
  changeValMax() {
      ...
      const horisental = oxClass.checkOxHorizen()
      ...
      const ggg = this.color_click.map(d => d.filter( d2 => d2 == "clicked").length).reduce((sum ,val) => sum + val , 0)
      // console.log({ggg})
      if(ggg === 9) {
        // console.log('dewss')
        this.dewsVal += 1
